          ...
          {METRICS.map((metric, i) => {
            ...
            return (
              <ScrollReveal key={metric.label} delay={i * 0.08}>
                <div
                  className="h-full rounded-2xl border border-stone-200/80 dark:border-stone-800/80 bg-white dark:bg-neutral-950 p-8 shadow-[0_16px_50px_-28px_rgba(0,0,0,0.06)] transition-transform duration-300 ease-out hover:-translate-y-1.5 motion-reduce:transform-none motion-reduce:transition-none"
                >
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-stone-600 dark:text-stone-400">{metric.description}</p>
                </div>
              </ScrollReveal>
            );
          })}
